...
def reserve_spot():
    ...
    existing_reservation = Reservation.query.filter_by(
        user_id=session["user_id"], checkout_time=None
    ).first()

    if existing_reservation:
        flash(
            "You already have an active reservation. Please check out first.", "warning"
        )
    ...
    if selected_lot_id:
        available_spots = ParkingSpot.query.filter_by(
            lot_id=selected_lot_id, is_occupied=False
        ).all()
...
def checkout(reservation_id):
    ...
    flash(
        f"Checked out successfully! Duration: {duration} hours, Cost: ${cost}",
        "success",
    )
...
def reservation_history():
    reservations = (
        ...
        .order_by(Reservation.checkin_time.desc())
        .all()
    )
...
def admin_dashboard():
    ...
    # Recent reservations
    recent_reservations = (
        Reservation.query.order_by(Reservation.checkin_time.desc()).limit(10).all()
    )
...
def add_lot():
    ...
    if form.validate_on_submit():
        ...
        for i in range(1, lot.total_spots + 1):
            spot = ParkingSpot(
                spot_number=f"{lot.name[:3].upper()}-{i:03d}", lot_id=lot.id
            )
            db.session.add(spot)
        ...
        flash(
            f'Parking lot "{lot.name}" with {lot.total_spots} spots added successfully!',
            "success",
        )
...
def manage_reservations():
    ...
    reservations = Reservation.query.order_by(Reservation.checkin_time.desc()).paginate(
...
def delete_lot(lot_id):
    ...
    if occupied_spots > 0:
        flash(
            f'Cannot delete lot "{lot.name}" - {occupied_spots} spots are currently occupied.',
            "danger",
        )
        return redirect(url_for("manage_lots"))
