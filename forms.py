...
class ParkingLotForm(FlaskForm):
    ...
    total_spots = IntegerField('Total Spots', validators=[DataRequired(), NumberRange(min=1, max=1000)])
    submit = SubmitField('Add Parking Lot')


class ReservationForm(FlaskForm):
    lot_id = SelectField('Select Parking Lot', coerce=int, validators=[DataRequired()])
    spot_id = SelectField('Select Parking Spot', coerce=int, validators=[DataRequired()])
    vehicle_number = StringField('Vehicle Number', validators=[
        DataRequired(),
        Length(max=20),
        Regexp(r'^[A-Z0-9\-\s]+$', message="Vehicle number should contain only letters, numbers, hyphens and spaces")
    ])
